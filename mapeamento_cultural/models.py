...
class Artista(models.Model):
    ...
    cpf=models.CharField(max_length=14, verbose_name='CPF', validators=[validate_CPF], blank=True, null=True)
...
class InformacoesExtras(models.Model):
    ...
    area=models.ManyToManyField(Area_Atuacao, blank=True)
    publico=models.ManyToManyField(Publico_Atuacao, blank=True)
    enquadramento=models.ManyToManyField(Enquadramento_Atuacao, blank=True)
    forma_atuacao=models.ManyToManyField(Forma_insercao_Atuacao, blank=True)
    qnt=models.CharField(max_length=1, choices=QNT_CHOICES, blank=True)
